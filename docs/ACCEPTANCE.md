...
| T20–T24 | 离线队列、多标签、版本冲突、会话与依赖保护 | 🧪 | T20 ✅（浏览器实测：断网入队→同步失败保留→恢复后入账且仅一次）；T18/T19 服务端幂等已 ✅；T22 ✅ 版本冲突细化（周期规则 base_version 乐观并发：不一致返回 409 version_conflict 且不落库，前端提示刷新重试，TestT22_RuleVersionConflict）；T23 会话过期暂停同步、世代不一致 409 已实测；T24 依赖排序为后续增强 |
...
| T47 | PWA | 🧪 | Manifest（192/512/maskable 生图图标）+ standalone + SW 应用壳缓存 + 离线回退实测；安装引导 UI ✅（设置页「安装应用」：Chromium 一键安装/iOS 引导分享添加/不支持时如实说明，按浏览器能力无假开关；iOS 真机验证待交付）；后台同步不可用设备的降级：回前台/联网/手动触发 |
